...
var Stats = function() {
  ...
  return {
    ...
    begin: function() {
      ...
      for (var i = 0; i < swQueue.length; i++) {
        if (time - swQueue[i][0] < swStep) {
          sumST = sumST + swQueue[i][1];
        } else {
          break;
        }
      }

      swPanel.update(sumST / 10000, Math.pow(swStep, 1.8) / 10000);

      if (swQueue.length > 1000) {
        swQueue = swQueue.slice(0, 1000);
      }
